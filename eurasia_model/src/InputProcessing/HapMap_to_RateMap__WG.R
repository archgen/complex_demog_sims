...
hapmap = rbindlist(lapply(1:22, function(x) fread(paste0("./HapMapData/genetic_map_HapMapII_GRCh37/genetic_map_GRCh37_chr", x, ".txt"))))
...
Figure_new_pos_cum_new_map_cum = ggplot(data = hapmap[plotID == TRUE], aes(x = new_pos_cum, y = new_map_cum))+
  geom_point()+
  xlab("Cumulative genomic physical position") + 
  ylab("Cumulative genomic map position")

Figure_new_pos_cum_new_rrate = ggplot(data = hapmap[plotID == TRUE], aes(x = new_pos_cum, y = new_rrate))+
  geom_point()+
  xlab("Cumulative genomic physical position") + 
  ylab("Recombination rate (cM/Mb)")

Figure_hapmap_ratemap = cowplot::plot_grid(Figure_new_pos_cum_new_map_cum, Figure_new_pos_cum_new_rrate)
ggsave(paste0("Figure_hapmap_ratemap",".jpeg"),  
       width = 15, height = 10, units = "in", dpi=300, Figure_hapmap_ratemap, 
       path = "./snakemake/project__complex_msp_simulation/input/hapmap/")
...
options(scipen = 999)
write.table(hapmap_out, file = "./snakemake/project__complex_msp_simulation/input/rate_map_genetic_map_HapMapII_GRCh37_log2recom.txt",
            quote = FALSE, row.names = FALSE)

write.table(hapmap_out_chr, file = "./snakemake/project__complex_msp_simulation/input/chr_rate_map_genetic_map_HapMapII_GRCh37_log2recom.txt",
            quote = FALSE, row.names = FALSE)
